 ...
 *
 * Alt regnes i minutter etter midnatt internt, og vises som «08:30». Å regne
 * på strengene direkte gikk ikke: «09:50» pluss 20 minutter er ikke «09:70».
 *
 * Uka er mandag til fredag med vilje. Samtaler settes opp i skoletida, og et
 * skjema med lørdag og søndag ville gitt to tomme spalter på arket læreren
 * henger opp.
 */

/** Ukedagene et oppsett kan bruke. Nøkkelen er tallet som lagres i `day`. */
...
export function slotsForDay(slots: MeetingSlot[], day: number): MeetingSlot[] {
  return sortSlots(slots.filter((s) => s.day === day));
}

/**
 * Bygger tidene på nytt fra skjemaet: fra `day_start`, én samtale om gangen
 * med `gap` mellom, til det ikke er plass til én til før `day_end`.
 *
...
export function buildSlots(plan: MeetingPlan): MeetingSlot[] {
  ...
  for (const day of [...plan.days].sort((a, b) => a - b)) {
    let at = from;
    let count = 0;
...
export function fillSlots(slots: MeetingSlot[], studentIds: string[]): MeetingSlot[] {
  ...
  const free = new Map<number, MeetingSlot[]>();
  for (const slot of sortSlots(slots)) {
    if (slot.student_id || slot.note.trim()) continue;
    const inDay = free.get(slot.day) ?? [];
    inDay.push(slot);
    free.set(slot.day, inDay);
  }
  const days = [...free.keys()].sort((a, b) => a - b);
 ...
 * halve klassen til en annen ukedag — og dagen er det første foresatte har
 * skrevet ned. Blir det færre tider på en dag enn det sto elever der, faller de
 * siste ned i den vanlige fordelingen.
 */
export function refill(fresh: MeetingSlot[], previous: MeetingSlot[]): MeetingSlot[] {
  ...
  for (const { day } of WEEKDAYS) {
    const students = slotsForDay(previous, day)
      .map((s) => s.student_id)
      .filter((id): id is string => Boolean(id));
    if (students.length === 0) continue;

    const open = slotsForDay(fresh, day).filter((s) => !s.student_id && !s.note.trim());
...
export function clashingSlots(slots: MeetingSlot[]): Set<string> {
  ...
  for (const { day } of WEEKDAYS) {
    const inDay = slotsForDay(slots, day);
...
/** Datoen en ukedag faller på, eller `null` når oppsettet er uten uke. */
export function dateForDay(weekStart: string, day: number): Date | null {
...
/** «Mandag», eller «Mandag 14. sep.» når oppsettet har en uke. */
export function dayLabel(weekStart: string, day: number): string {
  const name = WEEKDAYS.find((d) => d.day === day)?.name ?? "";
  const date = dateForDay(weekStart, day);
  if (!date) return name;
  return `${name} ${date.toLocaleDateString("nb-NO", { day: "numeric", month: "short" })}`;
}
      ...
      ? value.slots
          ...
          .map((s) => {
            ...
            return {
              ...
              day: Math.min(5, Math.max(1, Math.round(Number(s.day)) || 1)),
